...
template<class T>
bool GetRefThis(Context* context, JSValueConst this_val, RefPtr<T>* ptr,JSClassID cid) {
	...
	if (cid) {
		...
		while (pid) {
			if (pid == cid) {
				...
				*ptr = Ref(reinterpret_cast<T*>(JS_GetOpaque(this_val, id)));
				return *ptr;
			}
...
template<class T>
bool GetRefThis(JSValueConst this_val, RefPtr<T>* ptr) {
	if (!JS_IsObject(this_val)) {
		return false;
	}
	JSClassID id = JS_GetClassID(this_val);
...
template<class T>
inline void SetRefThis(JSValueConst this_val, RefPtr<T> pThis) {
	pThis->AddRef();
	JS_SetOpaque(this_val, pThis.get());
}

//RefClass js强引用
template<class T>
class RefClass :public ClassBase {
public:
	RefClass(JSContext* context, Module* module, const char* name)
	...
	//转为C指针，会校验classid
	static RefPtr<T> ToC(const Value& v) {
		auto context = v.context();
		if (!context) {
			return nullptr;
		}
		RefPtr<T> pThis;
		return GetRefThis(context, v, &pThis, class_id_) ? pThis : nullptr;
	}

	//创建新的js对象，注意js对象释放时会调用dtor释放ptr
	static Value ToJs(Context& context, RefPtr<T> ptr) {
		if (!ptr)
			return null_value;
	...
	static Value ToJsById(Context& context, RefPtr<T> ptr, JSClassID cid) {
		if (!ptr)
			return null_value;
	...
	template<RefPtr<T> ctor(Context& context, ArgList& args)>
	void AddCtor() {
		JSValue constructor = JS_NewCFunction2(context_, [](JSContext* ctx,
			...
			int argc, JSValueConst* argv) {
				...
				RefPtr<T> pThis = ctor(*context, arg_list);
				if (!pThis) {
	...
	template<RefPtr<T> ctor(Context& context, Value& this_obj, ArgList& args)>
	void AddCtor2() {
		JSValue constructor = JS_NewCFunction2(context_, [](JSContext* ctx,
			JSValueConst new_target,
			int argc, JSValueConst* argv) {
				...
				RefPtr<T> pThis = ctor(*context, obj, arg_list);
				if (!pThis) {
					return JS_ThrowInternalError(ctx, "ctor error");
				}
...
template<class T>
JSClassID RefClass<T>::class_id_ = 0;

template<class T>
RefClass<T> Module::ExportRefClass(const char* name) {
	return RefClass<T>(context_, this, name);
}
