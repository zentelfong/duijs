...
// subclassing |RefCounted| -- see ref_counted.h).
//
...
template <typename T>
static inline RefPtr<T> Ref(T* ptr) {
	return RefPtr<T>(ptr);
}
...
RefPtr<T> MakeRefCounted(Args&&... args) {
  return detail::MakeRefCountedHelper<T>::MakeRefCounted(
...
namespace std {
template <typename T>
struct hash<qjs::RefPtr<T>> {
  using argument_type = qjs::RefPtr<T>;
