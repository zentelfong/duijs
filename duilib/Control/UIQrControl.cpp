...
namespace DuiLib {
	...
	void CQrControl::SetText(LPCTSTR text)
	{
		m_text=text;
		DeleteObject(m_qrImage);
		m_qrImage=NULL;
	...
	void CQrControl::PaintStatusImage(HDC hDC)
	{
		if (!m_qrImage)
		{
	...
	void CQrControl::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
	...
	bool CQrControl::SaveImage(LPCTSTR path)
	{
		if (m_qrImage)
	...
	HBITMAP CQrControl::GenQrImage(HDC hdc,LPCTSTR text,int size)
	{
		CQR_Encode qrEncode;
