...
namespace DuiLib
{
	...
	CProgressUI::CProgressUI() : m_bShowText(false), m_bHorizontal(true), m_bXMirror(false), m_nMin(0), m_nMax(100), m_nValue(0), m_bStretchForeImage(true)
	{
    ...
    void CProgressUI::SetXMirror(bool bXMirror)
    {
        m_bXMirror = bXMirror;
    }
	...
	void CProgressUI::PaintForeColor(HDC hDC)
	{
		...
		if( m_bHorizontal ) {
            ...
            if (m_bXMirror)
                rc.top = m_rcItem.top + ((m_rcItem.bottom - m_rcItem.top) - (m_rcItem.bottom - m_rcItem.top) * (m_nValue) / (m_nMax - m_nMin));
            else
